...
- Node.js 24.14.1 以上（`package.json` の `engines.node` で `>=24.14.1` を指定。`engineStrict: true` のため、この下限を満たす Node.js を使うこと）
- pnpm 11以上
...
既存互換のため、引数なしの `kizami setup` は Claude Code のみを設定します。

Codex も使う場合は明示的に Codex target を追加します。
...
既に Claude Code に Kizami をインストール済みの場合、Claude Code の利用だけなら追加作業は不要です。
Codex でも記憶保存・注入を有効にしたい場合だけ `kizami setup --target codex` を実行してください。
...
- SessionStart hookでプロジェクト直近Q&Aを冒頭注入します
- UserPromptSubmit hookで関連記憶を自動注入し、同時に送信プロンプトを一時保存します
...
KizamiはClaude Code / Codex の hook で動作し、**JSONLを正本・SQLiteを派生キャッシュ**として二層管理します。
Claude Code では `SessionEnd` の `transcript_path` を正本化し、Codex では transcript 形式に依存せず `UserPromptSubmit.prompt` と `Stop.last_assistant_message` を hook payload から取り込みます。

```mermaid
flowchart TD
    subgraph CC["Claude Code / Codex"]
        SS["SessionStart hook (v0.2.0+)"]
        SE["SessionEnd hook (Claude) / Stop hook (Codex)"]
        UP["UserPromptSubmit hook"]
    end

    SS -->|"session_id, cwd (stdin JSON)"| INJECT["kizami inject"]
    SE -->|"transcript_path or last_assistant_message (stdin JSON)"| SAVE["kizami save"]
...
`kizami setup --target codex` を実行すると、Codex の `~/.codex/hooks.json` に Codex 用 hook が追加されます。
以下の例では `kizami` と表記していますが、ビルド済みCLIから `setup` を実行した場合は PATH 依存を避けるため `node /absolute/path/dist/cli.js` 形式で登録されることがあります。
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "kizami inject --stdin --runtime claude # kizami-managed"
          }
        ]
      }
    ]
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "kizami recall --stdin --runtime claude # kizami-managed"
          }
        ]
      }
    ]
...
Codex は Claude Code と異なり、終了時 hook は `SessionEnd` ではなく `Stop` です。
また Codex の `transcript_path` の中身は安定した外部インターフェースではないため、Kizami は Codex transcript を保存元として使いません。

```bash
kizami setup --target codex
kizami setup status --target codex
kizami setup uninstall --target codex
```

Codex に追加される hook は以下です。
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "kizami inject --stdin --runtime codex # kizami-managed",
            "timeout": 5
          }
        ]
      }
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "kizami recall --stdin --runtime codex # kizami-managed",
            "timeout": 5
          }
        ]
      }
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "kizami save --stdin --runtime codex # kizami-managed",
            "timeout": 5
          }
        ]
      }
...
Codex では `UserPromptSubmit` で検索注入を行うと同時に prompt を一時保存し、`Stop` で `last_assistant_message` と結合して1ターンとして保存します。
`turn_id` がある場合は session+turn単位で一時保存するため、同一セッション内の複数ターンも混線しません。
この保存経路でも JSONL への先書きが必須で、SQLite は派生キャッシュとして更新されます。hybridモードでは Claude Code 保存経路と同様に embedding を生成して JSONL と `chunks_vec` に反映します。
Kizami は `# kizami-managed` marker 付きの hook だけを置換・削除し、他ツールの hook は保持します。
...
| recall 1回あたりの注入量 | 最大約300トークン (3件 x 約100トークン) |
| Token節約率              | 99.98%                                  |
...
| ------------------ | ------------ |
| 全履歴を毎回注入   | 83,825,700   |
| Kizamiのrecall注入 | 約15,000     |
| 節約量             | 約83,810,700 |

全履歴を渡す方式ではコンテキストウィンドウに収まらないため現実的ではありません。Kizamiは関連度スコアによるフィルタリングと短い注入フォーマットにより、必要な記憶だけを小さく収めて注入します。
